...
function fetchAndReadCSV(fileUrl) {
  fetch(fileUrl)
    .then(response => {
      if (!response.ok) {
...
buttons.forEach(button => {
  button.addEventListener('click', () => {
    ...
    switch (button.id) {
      case 'stbtn1':
        sortByLikesDescending(agreeArray);
        break;
      case 'stbtn2':
        sortSerializedDatesDescending(agreeArray);

        // 執行其他排序邏輯
        break;
      case 'stbtn3':
        sortSerializedDatesAscending(agreeArray);
        // 執行自訂排序邏輯
        break;
    }
    renew();
...
buttons2.forEach(button => {
  button.addEventListener('click', () => {
    ...
    switch (button.id) {
      case 'stbtn4':
        console.log("T4");
        sortByLikesDescending(disagreeArray);
        break;
      case 'stbtn5':
        console.log("T5");
        sortSerializedDatesDescending(disagreeArray);
...
.addEventListener('change', (event) => {
  const file = event.target.files[0];
  readCSV(file, (data) => {
    console.log(data); // 這時 data 會是一個包含多個 commentData 物件的陣列
    // 你可以對 data 進行排序或其他操作
  });
});*/